...
#		except ValueError: self.kaks = None
	...
	def gene2species(self, gene_pair):
		sp1, sp2 = gene_pair if gene_pair==(None, None) else map(lambda x: x.split('|')[0], gene_pair)
...
def cluster_add_outgroup(collinearities, orthogroup, outgroup, fout=sys.stdout, min_ratio=0):
	...
	for og in OrthoMCLGroup(orthogroup):
		...
		nxg = len(og.genes)
		add_genes = []
		for g, count in Counter(outgrp_genes).items():
			ratio = 1.0*count/nxg
			if ratio > min_ratio:
				add_genes += [g]
				ng += 1
...
class ToAstral(ColinearGroups):
	def __init__(self, input, pep, spsd=None, cds=None, tmpdir='tmp', root=None, both=True, suffix=None, 
			...
			trimal_opts='-automated1', iqtree_opts='-B 1000'):
		self.input = input
		self.pep = pep
		self.cds = cds
		self.spsd = spsd
