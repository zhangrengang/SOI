...
class AK:
	...
	def plot(self, sort_by_sg=True, outfig=None, bar_width=0.5, #hatchs='/\|-+xoO.*'
		...
		if sort_by_sg:
			chromosomes = sorted(chromosomes, key=lambda x:x.longest_subgenome)
...
def stats_besthits(alignment, pol_indice, dip_indice, labels, blasts):
	...
	for polAln, dipAln in zip(polAlns, dipAlns):
		...
		for polG, polSP in zip(polAln.genes, polAln.species):
			...
			try: d_count[key1][key2] += 1
			except KeyError: 
				try: d_count[key1][key2] = 1
				except KeyError: d_count[key1] = {key2:1}
	print(x, 'errors;', y, 'valid', file=sys.stderr)
	line = ['sg_i', 'sg_lab', 'polyploid', 'prog_i', 'diploid', 'count', 'percent']
		...
		for sp, lines, idx in self.parse_by_species():
			...
			for line in lines:
				if line.missing_rate > max_missing:
					continue
				genes = [g if g is None else g.split('|', 1)[-1] for g in line.genes ]
				...
				for i, exp in enumerate(exps):
					if not norm and exp == 0:
						continue
					try: sg = self.idmap[idx[i]]
					except: sg = 'sg{}'.format(i)
	...
	def win_trees(self, pep, gff, win_size=50, win_step=10, min_ratio=0.7, min_seqs=4, tmpdir='tmp', exclude_ref=False, **kargs):
		...
		for chrom, xlines in itertools.groupby(lines, lambda x: d_genes[x.id].chrom):
			...
			for i in range(0, ngenes, win_step):
				...
				try: start, end = xlines[i], xlines[i+win_step]
				except IndexError: continue
				
				alnfiles = [d_alnfiles[line.id] for line in lines if line.id in d_alnfiles]
		...
		for (chrom, start, end), treefile in list(d_treefiles.items()):
			...
			block.topl = topo
			try: d_blocks[chrom] += [block]
			except KeyError: d_blocks[chrom] = [block]
			topos += [topo]
		f.close()
